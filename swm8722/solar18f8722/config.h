...
#define MAX_CHARGEAMPS	390		// don't overcharge battery with PV and charger on 
#define CHARGER_AMPS	250		// max current from the utility charger.
#define CHARGER_EQUAL	20		// number of full cycles before an equalization charge is flagged
...
#define B3AH		10      	// AGM cell, if these are changed for S to M or back the EEPROM must be reset with SW1
#define	B4AH		10      	// AGM cell
#define B1ID		'M'     	// FLA costco GC2 or large AGM
#define	B2ID		'M'    		// FLA costco CG2 or large AGM
...
#define	WCHARGER	17000.0 	// turn on charger if battery weight on inverter is below this point
...
#define CC_RESET_MAX	2		// Max amount of times to reset the Charge controller after float to rebulk
