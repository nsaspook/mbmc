...
{
	BYTE i;
	static BYTE EOM = 0, MyBuffer[64] = {0};
	WORD w, w2, num_data;
	BYTE AppBuffer[32];
	...
	case SM_LISTENING:
		...
		for (w = 0; w < wMaxGet; w += sizeof(AppBuffer)) {
			...
			// Look for connection terminate esc character
			for (w2 = 0; w2 < wCurrentChunk; w2++) {
				i = AppBuffer[w2];
				if (i == 'X') { // term char for data
					EOM = 1;
				}
				if (i == 0x1B) { //escape
			...
			TCPPutArray(MySocket, (BYTE*) "D ", 1); // Data code
			//			TCPPutArray(MySocket, AppBuffer, wCurrentChunk);
			strncat((char*) MyBuffer, (char*) AppBuffer, wCurrentChunk);
		}

		if (EOM) {
			EOM = 0;
			TCPPutArray(MySocket, (BYTE*) "R ", 1); // received code
			num_data = sscanf((char*) MyBuffer, "%ld,%li,%i,%i,", &remote_daq.inputvoltage, &remote_daq.inputcurrent,
				&remote_daq.sequence, &remote_daq.station);
			MyBuffer[0] = 0;
			if (num_data == 4) {
				TCPPutArray(MySocket, (BYTE*) "V ", 1); // Valid code
				// valid data we hope
				remote_daq.rawtime = ltime; // create time stamp
				remote_daq.new = TRUE; // we have data from a remote station
				if (remote_daq.read) { // update soon after a main controller update with fresh data
					mPORTDToggleBits(BIT_1); // flash led on/off when receiving remote data
					remote_daq.read = FALSE; // clear the new data flag
					remote_daq.voltagedrop = remote_daq.inputvoltage - mbmc_buffer.inputvoltage;
					if (((long) remote_daq.inputvoltage - (long) mbmc_buffer.inputvoltage) < 0) remote_daq.voltagedrop = 0;
					remote_daq.remotecurrent = mbmc_buffer.currentin - remote_daq.inputcurrent;
					if ((mbmc_buffer.currentin - remote_daq.inputcurrent) < 0) remote_daq.remotecurrent = 0;
					remote_daq.offline = FALSE;
				}
			} else {
				remote_daq.rawtime = 0;
				remote_daq.inputvoltage = remote_daq.inputcurrent = 0; // set to all zeros
				remote_daq.read = FALSE; // clear the new data flag
				TCPPutArray(MySocket, (BYTE*) "I ", 1); // Invalid code
			}
		}
